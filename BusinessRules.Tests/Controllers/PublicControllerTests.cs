...
";
        ...
        [TestMethod()]
        public async Task ExecuteRules_IfOfflineModeEnabled_ResultIsLoggedSuccessfullyForRuleExecution()
        {
...
";
            ...
            await service.SaveRule(company.Id, newRule1);
            var apiKey1 = new BizApiKey(company.Id, convertedField1.Id);
            apiKey1.LocalLoggingDirectory = Path.GetTempPath();
            await service.SaveApiKey(apiKey1);
...
";
            ...
            var foundFiles =
                Directory.GetFiles(Path.GetTempPath()).Where(x => x.Contains(apiKey1.Id.ToString("N")))
                .ToList();

            var tempPathContainsLogFiles =
                Directory.GetFiles(Path.GetTempPath()).Any(x => x.Contains(apiKey1.Id.ToString("N")));

            Assert.IsTrue(tempPathContainsLogFiles);

            FeatureFlags.OfflineMode = false;
        }

        [TestMethod()]
        public async Task ExecuteRules_IfOfflineModeDisabled_ResultIsLoggedSuccessfullyForRuleExecution()
        {
            var service = new BusinessRulesService(_tableServiceClient, _backgroundStorageRepository, _config);
            var httpContextAccessor = new HttpContextAccessor
            {
...
";
            ...
            var apiKey1 = new BizApiKey(company.Id, convertedField1.Id);
            apiKey1.LocalLoggingDirectory = Path.GetTempPath();
            apiKey1.FtpsServer = "127.0.0.1";
            apiKey1.FtpsPort = "9990";
            apiKey1.FtpsUsername = "bogus";
            apiKey1.FtpsPassword = "bogus";
            apiKey1.FtpsRemoteDirectory = "/bogus";
            await service.SaveApiKey(apiKey1);
...
";
        ...
        [TestMethod()]
        public async Task ExecuteRules_IfAnArrayOfTopLevelFieldsIsSent_RulesExecuteSuccessfully()
        {
            var service = new BusinessRulesService(_tableServiceClient, _backgroundStorageRepository, _config);
            var httpContextAccessor = new HttpContextAccessor
            {
...
";
            ...
            var result =
                await publicController.ExecuteRules(apiKey.Id, testFieldArray) as ContentResult;
            Assert.IsTrue(result.Content.ToString().Contains("wonky"));
        }
    }
